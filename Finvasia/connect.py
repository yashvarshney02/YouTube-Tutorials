...
import os
import pandas as pd
import Finvasia.finvasia_constants as fc
...
class NorenApi:
    ...
    def __init__(self, host, websocket, userid, password, twoFA, apikey):
        ...
        self.__factor2 = twoFA
        self.__apikey = apikey
        self.__vc = f"{self.__username}_U"
        self.__imei = "abc1234"
    ...
    def __ws_run_forever(self):
        ...
        while not self.__stop_event.is_set():
            try:
                self.__websocket.run_forever(ping_interval=3)
    ...
    def __on_open_callback(self, ws=None):
        ...
        # prepare the data
        values = {"t": "c", "uid": self.__username, "actid": self.__username, "susertoken": self.__susertoken,
                  "source": 'API'}
    ...
    def start_websocket(self, subscribe_callback=None,
        ...
        url = self.__service_config['websocket_endpoint'].format(self.__susertoken)
    ...
    def check_invalid_token(self, response):
        if type(response) == list:
            return False
        if response.get('stat') == 'Not_Ok' and response.get('emsg') == fc.INVAILD_TOKEN:
            print("re setting session")
            sleep(0.2)
            # reset the session procedure like reading from local file/ creating new login flow
            self.set_session()
            return True
        return False
    ...
    def login(self):
        ...
        pwd = hashlib.sha256(self.__password.encode('utf-8')).hexdigest()
        u_app_key = '{0}|{1}'.format(self.__username, self.__apikey)
        app_key = hashlib.sha256(u_app_key.encode('utf-8')).hexdigest()
        # prepare the data
        values = {"source": "API", "apkversion": "1.0.0", "uid": self.__username, "pwd": pwd,
                  "factor2": pyotp.TOTP(self.__factor2).now(), "vc": self.__vc, "appkey": app_key, "imei": self.__imei}
        ...
        # Yash
        # error: {'stat': 'Not_Ok', 'emsg': 'Invalid Input :  Invalid App Key'}
        if resDict['stat'] != 'Ok':
            raise Exception(resDict['emsg'])
    ...
    def verify_token(self, token):
        ...
        if type(resDict) == list:
            return False
        if resDict.get('stat') == 'Not_Ok' and resDict.get('emsg') == fc.INVAILD_TOKEN:
            return False
        return True
    ...
    def place_order(self, buy_or_sell, product_type,
        ...
        if self.check_invalid_token(resDict):
            return self.place_order(buy_or_sell, product_type,
                                    exchange, tradingsymbol, quantity, discloseqty,
    ...
    def cancel_order(self, orderno):
        ...
        resDict = json.loads(res.text)
        if self.check_invalid_token(resDict):
            return self.cancel_order(orderno)
        # Yash
        if resDict['stat'] != 'Ok':
            return resDict
    ...
    def get_basket_margin(self, first_order, remaining_orders):
        """
        :param first_order: first order of the basket
        :param remaining_orders: remaining orders of the basket
        :return: a dict with margin used and remarks

        Always put every thing in string format
        sample success
        {'request_time': '23:00:13 15-05-2023', 'stat': 'Ok', 'marginused': '174967.75', 'marginusedtrade': '174967.75', 'remarks': ''}
        """
        config = NorenApi.__service_config

        # prepare the uri
        url = f"{config['host']}{config['routes']['basketmargin']}"
        print(url)

        values = {'uid': self.__username, 'actid': self.__username}
        values.update(first_order)
        values[fc.BASKETLISTS] = remaining_orders
        ...
        if self.check_invalid_token(resDict):
            return self.get_basket_margin(first_order, remaining_orders)
    ...
    def single_order_history(self, orderno):
        ...
        if self.check_invalid_token(resDict):
            return self.single_order_history(orderno)
        # error is a json with stat and msg which we printed earlier.
        # Yash

        # it will simply return the list of different dictionaries with one by one history element
        return resDict

    def get_order_book(self, tries=0, error=None):
        config = NorenApi.__service_config
        ...
        try:
            ...
            return self.get_order_book(tries+1, requests.exceptions.Timeout)
        reportmsg(res.text)
        ...
        if str(type(resDict)) == "<class 'dict'>" and resDict.get('stat') == 'Not_Ok' and resDict[
    ...
    def get_time_price_series(self, exchange, token, starttime=None, endtime=None, interval=None):
        ...
        #
        values = {'ordersource': 'API', "uid": self.__username, "exch": exchange, "token": token, "st": str(starttime)}
    ...
    def get_holdings(self, product_type=None):
        ...
        if str(type(resDict)) == "<class 'dict'>" and resDict.get('stat') == 'Not_Ok' \
                and resDict['emsg'] == 'Error Occurred : 5 "no data"':
            return []
    ...
    def get_limits(self, product_type=None, segment=None, exchange=None):
        ...
        if self.check_invalid_token(resDict):
            return self.get_limits(product_type, segment, exchange)
